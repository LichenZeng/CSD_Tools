 ...
 */
public class FmRadioService extends Service implements
        FmRecorder.OnRecorderStateChangedListener {
    // Logging
    ...
    private static String sRecordingSdcard = FmRadioUtils
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_TOFMSERVICE_POWERDOWN.equals(action)
                    ...
                    || (SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE
                            .equals(command))) {
                ...
                if (SHORT_ANNTENNA_SUPPORT) {
                    boolean isSwitch = (switchAntenna(mValueHeadSetPlug) == 0) ? true
                            : false;
                    ...
                    // Notify UI change to earphone mode, false means not
                    // speaker mode
                    Bundle bundle = new Bundle(2);
                    bundle.putInt(FmRadioListener.CALLBACK_FLAG,
                            FmRadioListener.LISTEN_SPEAKER_MODE_CHANGED);
                    bundle.putBoolean(FmRadioListener.KEY_IS_SPEAKER_MODE,
                            !plugInEarphone);
                    ...
                    if (!mIsServiceInited) {
                        Log.d(TAG,
                                "onReceive.switch anntenna:service is not init");
                        powerUpAutoIfNeed();
                        return;
                    }
                    /*
                     * If ear phone insert and activity is foreground. power up
                     * FM automatic
                     */
                    if ((0 == mValueHeadSetPlug) && isActivityForeground()) {
                        Log.d(TAG,
                                "onReceive.switch anntenna:need auto power up");
                        powerUpAsync(FmRadioUtils
                                .computeFrequency(mCurrentStation));
                    } else if (1 == mValueHeadSetPlug) {
                        Log.d(TAG, "plug out earphone, need to stop fm");
                        // ALPS01687760 Avoid sound from speaker after plug out
                        // earphone when recording
                        // plug out earphone will power down or exit, need to
                        // mute first anyway
                        setMute(true);
                        mFmServiceHandler
                                .removeMessages(FmRadioListener.MSGID_SCAN_FINISHED);
                        mFmServiceHandler
                                .removeMessages(FmRadioListener.MSGID_SEEK_FINISHED);
                        mFmServiceHandler
                                .removeMessages(FmRadioListener.MSGID_TUNE_FINISHED);
                        mFmServiceHandler
                                .removeMessages(FmRadioListener.MSGID_POWERDOWN_FINISHED);
                        mFmServiceHandler
                                .removeMessages(FmRadioListener.MSGID_POWERUP_FINISHED);
                    ...
                    .equals(action)) {
                int connectState = intent.getIntExtra(
                        BluetoothA2dp.EXTRA_STATE, 0);
                Log.d(TAG, "ACTION_CONNECTION_STATE_CHANGED: connectState="
                        + connectState + ", ispowerup=" + mIsPowerUp);
    ...
    /**
     * ALPS01756692 No sound after click FM app and power key Need to power up
     * auto for two cases: case 1: Launcher click FM app, then quickly click
     * Power key to lock phone. case 2: Launcher click FM app, then quickly
     * click Home key. Because power up action is in
     * FmRadioActivity.onServiceConnected(), these two cases will not callback
     * onServiceConnected() cause FmRadioActivity.onStop() has called unbind()
     */
    private void powerUpAutoIfNeed() {
        if ((0 == mValueHeadSetPlug)) {
            if (!mIsPowerUping && !mIsPowerUp && sActivityIsOnStop) {
                ...
                int iCurrentStation = FmRadioStation
    ...
    public boolean isBtConnected() {
        ...
        Log.d(TAG,
                "isBtConnected headset:"
                        + btAdapter
                                .getProfileConnectionState(BluetoothProfile.HEADSET)
                        + ", a2dp:"
                        + btAdapter
                                .getProfileConnectionState(BluetoothProfile.A2DP));
        int a2dpState = btAdapter
                .getProfileConnectionState(BluetoothProfile.HEADSET);
        return (BluetoothProfile.STATE_CONNECTED == a2dpState || BluetoothProfile.STATE_CONNECTING == a2dpState);
    }
    ...
    public void setSpeakerPhoneOn(boolean isSpeaker) {
        ...
        mForcedUseForMedia = isSpeaker ? AudioSystem.FORCE_SPEAKER
    ...
    private boolean openDevice() {
        ...
        Log.d(TAG, "<<< FmRadioService.openDevice: " + FmRadioNative.getHardwareVersion());
        Log.d(TAG, "<<< FmRadioService.openDevice: " + FmRadioNative.setMute(true));
        Log.d(TAG, "<<< FmRadioService.openDevice: " + FmRadioNative.getFmStatus(0));
    ...
    public void powerUpAsync(float frequency) {
        ...
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_POWERUP_FINISHED);
        msg.setData(bundle);
    ...
    private boolean powerUpFm(float frequency) {
        ...
        if (mIsPowerUp) {
            Log.d(TAG, "<<< FmRadioService.powerUp: already power up:"
                    + mIsPowerUp);
            return true;
        }
    ...
    private void waitIfTxSearching() {
        Log.d(TAG,
                ">>> waitIfTxSearching "
                        + FmRadioNative.getFmStatus(CURRENT_TX_SCAN));
    ...
    public void powerDownAsync() {
        ...
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_POWERUP_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_POWERDOWN_FINISHED);
    }
    ...
    public void tuneStationAsync(float frequency) {
        ...
        bundle.putFloat(FM_FREQUENCY, frequency);
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_TUNE_FINISHED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    public void seekStationAsync(float frequency, boolean isUp) {
        ...
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_SEEK_FINISHED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    private int[] startScan() {
        ...
        if (!mIsStopScanCalled) {
            ...
            Log.d(TAG,
                    "startScan native method:end "
                            + Arrays.toString(shortChannels));
        ...
        Log.d(TAG,
                "<<< FmRadioService.startScan: " + Arrays.toString(iChannels));
        return iChannels;
    }
    ...
    public void setRdsAsync(boolean on) {
        final int bundleSize = 1;
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_SET_RDS_FINISHED);
        Bundle bundle = new Bundle(bundleSize);
        bundle.putBoolean(OPTION, on);
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_SET_RDS_FINISHED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    public void activeAFAsync() {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_ACTIVE_AF_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_ACTIVE_AF_FINISHED);
    }
    ...
    public void setMuteAsync(boolean mute) {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_SET_MUTE_FINISHED);
        final int bundleSize = 1;
        Bundle bundle = new Bundle(bundleSize);
        bundle.putBoolean(OPTION, mute);
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_SET_MUTE_FINISHED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    public void switchAntennaAsync(int antenna) {
        ...
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_SWITCH_ANNTENNA);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    public void startRecordingAsync() {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_STARTRECORDING_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_STARTRECORDING_FINISHED);
    }
    ...
    private boolean isSdcardReady(String sdcardPath) {
        ...
        if (!mSdcardStateMap.isEmpty()) {
            if (mSdcardStateMap.get(sdcardPath) != null
                    && !mSdcardStateMap.get(sdcardPath)) {
    ...
    public void stopRecordingAsync() {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_STOPRECORDING_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_STOPRECORDING_FINISHED);
    }
    ...
    public void startPlaybackAsync() {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_STARTPLAYBACK_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_STARTPLAYBACK_FINISHED);
    }
    ...
    public void stopPlaybackAsync() {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_STOPPLAYBACK_FINISHED);
        mFmServiceHandler
                .sendEmptyMessage(FmRadioListener.MSGID_STOPPLAYBACK_FINISHED);
    }
    ...
    public void saveRecordingAsync(String newName) {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_SAVERECORDING_FINISHED);
        final int bundleSize = 1;
        Bundle bundle = new Bundle(bundleSize);
        bundle.putString(RECODING_FILE_NAME, newName);
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_SAVERECORDING_FINISHED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    public void setRecordingModeAsync(boolean isRecording) {
        mFmServiceHandler
                .removeMessages(FmRadioListener.MSGID_RECORD_MODE_CHANED);
        final int bundleSize = 1;
        Bundle bundle = new Bundle(bundleSize);
        bundle.putBoolean(OPTION, isRecording);
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_RECORD_MODE_CHANED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    private boolean initFmPlayer() {
        ...
        mFmPlayer.setWakeMode(FmRadioService.this,
    ...
    private void exitFm() {
        ...
        if (null != mFmRecorder) {
            synchronized (mStopRecordingLock) {
                ...
                // case 1: FileManager play Music->FM receive short audio
                // focus->Fm stop recording
                // -> FM receive SOUND_POWER_DOWN_MSG->Come here but is IDLE
                // status
                // case 2: Music play a song->FM receive long audio focus->Come
                // here is RECORDING
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d(TAG, ">>> FmRadioService.onStartCommand intent: " + intent
                + " startId: " + startId);
        ...
        mRdsThread = new Thread() {
            public void run() {
                ...
                while (true) {
                    ...
                    if (iRdsEvents != 0) {
                        Log.d(TAG, "FmRadioNative.readrds events: "
                                + iRdsEvents);
                    }
                    ...
                    if (RDS_EVENT_AF == (RDS_EVENT_AF & iRdsEvents)) {
                        ...
                        if (mIsScanning || mIsSeeking) {
                            Log.d(TAG,
                                    "RDSThread. seek or scan going, no need to tune here");
                        } else if (!mIsPowerUp) {
                            Log.d(TAG,
                                    "RDSThread. fm is power down, do nothing.");
                        } else {
                            ...
                            if (FmRadioUtils.isValidStation(iFreq)) {
                                ...
                                if (mCurrentStation == iFreq) {
                                    Log.w(TAG,
                                            "RDSThread. the new freq is the same as current.");
                                } else {
                                    setPS("");
                                    setLRText("");
                                    if (!mIsScanning && !mIsSeeking) {
                                        Log.d(TAG,
                                                "RDSThread. seek or scan not going,"
                                                        + "need to tune here");
                                        tuneStationAsync(FmRadioUtils
                                                .computeFrequency(iFreq));
                                    }
                                }
                            }
                        }
    ...
    private void setPS(String ps) {
        ...
        if (0 != mPSString.compareTo(ps)) {
            ...
            bundle.putInt(FmRadioListener.CALLBACK_FLAG,
                    FmRadioListener.LISTEN_PS_CHANGED);
    ...
    private void setLRText(String lrtText) {
        Log.d(TAG, "FmRadioService.setLRText: " + lrtText + " ,current: "
                + mLRTextString);
        if (0 != mLRTextString.compareTo(lrtText)) {
            mLRTextString = lrtText;
            Bundle bundle = new Bundle(3);
            bundle.putInt(FmRadioListener.CALLBACK_FLAG,
                    FmRadioListener.LISTEN_RT_CHANGED);
            bundle.putString(FmRadioListener.KEY_PS_INFO, mPSString);
            bundle.putString(FmRadioListener.KEY_RT_INFO, mLRTextString);
            notifyActivityStateChanged(bundle);
    ...
    private void showNotification() {
        ...
        notificationIntent.setClassName(getPackageName(),
                FmRadioActivity.class.getName());
        PendingIntent pendingIntent = PendingIntent.getActivity(
                getApplicationContext(), 0, notificationIntent, 0);
        Notification notification = new Notification(R.drawable.fm_title_icon,
        ...
        notification.setLatestEventInfo(getApplicationContext(), getResources()
                .getString(R.string.app_name), text, pendingIntent);
    ...
    public void onRecorderError(int error) {
        ...
        mRecorderErrorType = (MediaPlayer.MEDIA_ERROR_SERVER_DIED == error) ? FmRecorder.ERROR_PLAYER_INTERNAL
                : error;
    ...
    /**
     * Check and go next(play or show tips) after recorder file play back
     * finish. Two cases: 1. With headset or support short antenna -> play FM 2.
     * Without headset -> show plug in earphone tips
     */
    private void checkAfterPlayback() {
        if (isHeadSetIn() || SHORT_ANNTENNA_SUPPORT) {
            ...
            Log.d(TAG,
                    "checkAfterPlayback:earphone is out, need show plug in earphone tips");
            switchAntennaAsync(mValueHeadSetPlug);
        }
    }
    ...
    private final MediaPlayer.OnErrorListener mPlayerErrorListener = new MediaPlayer.OnErrorListener() {
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            ...
            if (MediaPlayer.MEDIA_ERROR_SERVER_DIED == what) {
                ...
                try {
                    mFmPlayer
                            .setDataSource("THIRDPARTY://MEDIAPLAYER_PLAYERTYPE_FM");
                    mFmPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
                    if (mIsPowerUp) {
    ...
    private final OnAudioFocusChangeListener mAudioFocusChangeListener = new OnAudioFocusChangeListener() {
        ...
        public void onAudioFocusChange(int focusChange) {
            ...
            default:
                Log.d(TAG,
                        "AudioFocus: Audio focus change, but not need handle");
                break;
            }
        }
    };
    ...
    private synchronized void updateAudioFocusAync(int focusState) {
        ...
        Message msg = mFmServiceHandler
                .obtainMessage(FmRadioListener.MSGID_AUDIOFOCUS_CHANGED);
        msg.setData(bundle);
        mFmServiceHandler.sendMessage(msg);
    }
    ...
    private void updateAudioFocus(int focusState) {
        ...
        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
            ...
            Log.d(TAG,
                    "AUDIOFOCUS_LOSS_TRANSIENT: mPausedByTransientLossOfFocus:"
        ...
        case AudioManager.AUDIOFOCUS_GAIN:
            ...
            if (!mIsPowerUp && mPausedByTransientLossOfFocus) {
                ...
                mFmServiceHandler
                        .removeMessages(FmRadioListener.MSGID_POWERDOWN_FINISHED);
                Bundle bundle = new Bundle(bundleSize);
                bundle.putFloat(FM_FREQUENCY,
                        FmRadioUtils.computeFrequency(mCurrentStation));
     ...
     * 
     * @param bundle
     *            The message to activity
     */
    private void notifyActivityStateChanged(Bundle bundle) {
        if (!mRecords.isEmpty()) {
            Log.d(TAG,
                    "notifyActivityStatusChanged:clients = " + mRecords.size());
            synchronized (mRecords) {
    ...
    public boolean isRecordingCardUnmount(Intent intent) {
        ...
        return unmountSDCard.equalsIgnoreCase("file://" + sRecordingSdcard) ? true
                : false;
    }

    private int[] insertSearchedStation(int[] channels) {
        Log.d(TAG,
                "insertSearchedStation.firstValidChannel:"
        ...
        if (null != channels) {
            ...
            for (int i = 0; i < size; i++) {
                if (FmRadioUtils.isValidStation(channels[i])) {
                    ...
                    if (!FmRadioStation
                            .isFavoriteStation(mContext, channels[i])) {
                        ops.add(ContentProviderOperation
                                .newInsert(Station.CONTENT_URI)
                                .withValue(Station.COLUMN_STATION_NAME,
                                        defaultStationName)
                                .withValue(Station.COLUMN_STATION_FREQ,
                                        channels[i])
        ...
        Log.d(TAG, "insertSearchedStation.firstValidChannel:"
                + firstValidChannel + ",channelNum:" + channelNum);
    ...
    class FmRadioServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case FmRadioListener.MSGID_SWITCH_ANNTENNA:
                bundle = msg.getData();
                int value = bundle
                        .getInt(FmRadioListener.SWITCH_ANNTENNA_VALUE);

                // if not support short antenna, just notify, not need to switch
                // antenna.
                if (SHORT_ANNTENNA_SUPPORT) {
                    isSwitch = (switchAntenna(value) == 0) ? true : false;
                    Log.d(TAG, "FmServiceHandler.switch anntenna:isSwitch:"
                            + isSwitch);
                } else {
                    ...
                    if (0 == value) {
                        Log.d(TAG,
                                "FmServiceHandler.switch anntenna:dismiss dialog");
                        // powerUpAsync(FMRadioUtils.computeFrequency(mCurrentStation));
                        bundle.putInt(FmRadioListener.CALLBACK_FLAG,
                                FmRadioListener.MSGID_SWITCH_ANNTENNA);
                        bundle.putBoolean(
                                FmRadioListener.KEY_IS_SWITCH_ANNTENNA, true);
                        notifyActivityStateChanged(bundle);
                    } else {
                        ...
                        if (mRecordState != FmRecorder.STATE_PLAYBACK) {
                            Log.d(TAG,
                                    "FmServiceHandler.switch anntenna:show dialog");
                            bundle.putInt(FmRadioListener.CALLBACK_FLAG,
                                    FmRadioListener.MSGID_SWITCH_ANNTENNA);
                            bundle.putBoolean(
                                    FmRadioListener.KEY_IS_SWITCH_ANNTENNA,
                                    false);
                            notifyActivityStateChanged(bundle);
                        }
                    }
            ...
            case FmRadioListener.MSGID_TUNE_FINISHED:
                ...
                bundle.putFloat(FmRadioListener.KEY_TUNE_TO_STATION,
                        tuneStation);
            ...
            case FmRadioListener.MSGID_SCAN_FINISHED:
                ...
                if ((null != channels) && channels[0] == -100) {
                    ...
                    isTune = tuneStation(FmRadioUtils
                            .computeFrequency(scanTuneStation));
                    scanTuneStation = isTune ? scanTuneStation
                            : mCurrentStation;
                }

                /*
                 * if there is stop command when scan, so it needs to mute fm
                 * avoid fm sound come out.
                 */
                if (mIsAudioFocusHeld) {
                ...
                bundle.putInt(FmRadioListener.KEY_TUNE_TO_STATION,
                        scanTuneStation);
            ...
            case FmRadioListener.MSGID_AUDIOFOCUS_CHANGED:
                bundle = msg.getData();
                int focusState = bundle
                        .getInt(FmRadioListener.KEY_AUDIOFOCUS_CHANGED);
    ...
    /**
     * Check if current is lock task mode. If in this mode, AMS will cannot
     * destory FmRadioActivity even call finish() Settings->Security->Screen
     * pinning on
     * 
     ...
     * 
     * @param listener
     *            The listener want to know the exit event
     */
     ...
     * 
     * @param name
     *            The latest recording name or null for not modified
     */
    ...
    /**
     * When FmRadioActivity.onStop() set true, FmRadioActivity.onResume() set
     * false;
     * 
     * @param stop
     */
     ...
     * 
     * @param index
     *            flag which will execute (0:rssi threshold,1:desense rssi
     *            threshold,2: SGM threshold)
     * @param value
     *            send to native
     * @return execute ok or not
     */
    public boolean setEmth(int index, int value) {
        Log.d(TAG, ">>> FMRadioService.setEmth: index=" + index + ",value="
                + value);
