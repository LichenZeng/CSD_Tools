...
public class FmRadioStation {
    ...
    public static final String AUTHORITY = "com.mediatek.fmradio.FmRadioContentProviderr";
    ...
    static final String COLUMNS[] = new String[] { Station._ID,
            Station.COLUMN_STATION_NAME, Station.COLUMN_STATION_FREQ,
    ...
    public static void initFmDatabase(Context context) {
        ...
        // FIXED_STATION_FREQ
        Cursor cur = context.getContentResolver().query(Station.CONTENT_URI,
                new String[] { Station.COLUMN_STATION_FREQ },
                Station.COLUMN_STATION_TYPE + "=?",
                new String[] { String.valueOf(STATION_TYPE_CURRENT) }, null);
        if (null != cur) {
            try {
                if (!cur.moveToFirst()) {
                    // Can't find current station, insert FIXED_STATION_FREQ to
                    // db
                    final int size = 3;
                    ContentValues values = new ContentValues(size);
                    values.put(Station.COLUMN_STATION_NAME,
                            CURRENT_STATION_NAME);
                    values.put(Station.COLUMN_STATION_FREQ,
                            FmRadioUtils.DEFAULT_STATION);
                    values.put(Station.COLUMN_STATION_TYPE,
                            STATION_TYPE_CURRENT);
                    context.getContentResolver().insert(Station.CONTENT_URI,
                            values);
                }
            } finally {
                cur.close();
        ...
        // Init PSRT, AF and TA
        int[] types = new int[] { RDS_SETTING_FREQ_PSRT, RDS_SETTING_FREQ_AF,
                RDS_SETTING_FREQ_TA };
        boolean[] enables = new boolean[] { DEFAULT_PSRT_ENABLED,
        ...
        for (int i = 0; i < types.length; i++) {
            cur = context.getContentResolver().query(
                    ...
                    Station.COLUMN_STATION_FREQ + "="
                            + String.valueOf(types[i]), null, null);
            if (null != cur) {
                try {
                    if (!cur.moveToFirst()) {
                        final int size = 3;
     ...
     *            The context
     * @param stationName
     *            The new station's name
     * @param oldStationFreq
     *            The old station frequency
     * @param newStationFreq
     *            The new station frequency
     * @param stationType
     *            The old station type
     */
    public static void updateStationToDb(Context context, String stationName,
            int oldStationFreq, int newStationFreq, int stationType) {
        final int size = 3;
        ContentValues values = new ContentValues(size);
        values.put(Station.COLUMN_STATION_NAME, stationName);
        ...
        context.getContentResolver().update(
                ...
                Station.COLUMN_STATION_FREQ + "=? AND "
                        + Station.COLUMN_STATION_TYPE + "=?",
                new String[] { String.valueOf(oldStationFreq),
     ...
     *            The station new name
     * @param newStationType
     *            The station new type
     * @param stationFreq
     *            The original station frequency
     */
    public static void updateStationToDb(Context context,
            String newStationName, int newStationType, int stationFreq) {
        final int size = 3;
        ContentValues values = new ContentValues(size);
        ...
        Log.d(TAG, "FmRadioStation.updateStationToDb: new name = "
                + newStationName + ", new freq type = " + newStationType);
    }
     ...
     *            The station type
     */
    public static void deleteStationInDb(Context context, int stationFreq,
            int stationType) {
     ...
     */
    public static boolean isStationExist(Context context, int stationFreq,
            int stationType) {
        ...
        Cursor cur = context.getContentResolver().query(
                ...
                Station.COLUMN_STATION_FREQ + "=? AND "
                        + Station.COLUMN_STATION_TYPE + "=?",
                new String[] { String.valueOf(stationFreq),
                        String.valueOf(stationType) }, null);
        if (null != cur) {
            try {
                if (cur.moveToFirst()) {
     ...
     * @return true or false indicate whether station is exist
     */
    public static boolean isStationExistInChList(Context context,
            int stationFreq) {
        Log.d(TAG, ">>> isStationExist: stationFreq=" + stationFreq);
        boolean isExist = false;
        Cursor cur = context.getContentResolver().query(
                ...
                new String[] { Station.COLUMN_STATION_NAME },
                Station.COLUMN_STATION_FREQ + "=? AND "
                        + Station.COLUMN_STATION_TYPE + "<>1",
                new String[] { String.valueOf(stationFreq) }, null);
        if (null != cur) {
            try {
                if (cur.moveToFirst()) {
                    // This station is exist
    ...
    public static void setCurrentStation(Context context, int station) {
        ...
        context.getContentResolver().update(
                Station.CONTENT_URI,
                values,
                Station.COLUMN_STATION_NAME + "=? AND "
                        + Station.COLUMN_STATION_TYPE + "=?",
                new String[] { CURRENT_STATION_NAME,
    ...
    public static void cleanSearchedStations(Context context) {
        ...
        context.getContentResolver().delete(
                Station.CONTENT_URI,
                Station.COLUMN_STATION_TYPE + "="
                        + String.valueOf(STATION_TYPE_SEARCHED), null);
     ...
     */
    public static String getStationName(Context context, int stationFreq,
            int stationType) {
        Log.d(TAG, "FmRadioStation.getStationName: type = " + stationType
                + ", freq = " + stationFreq);
        ...
        Log.d(TAG, "FmRadioStation.getStationName: stationName = "
                + stationName);
        return stationName;
    }
    ...
    public static int getStationCount(Context context, int stationType) {
        ...
        int stationNus = 0;
        Cursor cur = context.getContentResolver().query(Station.CONTENT_URI,
                COLUMNS, Station.COLUMN_STATION_TYPE + "=?",
                new String[] { String.valueOf(stationType) }, null);
        if (null != cur) {
            try {
